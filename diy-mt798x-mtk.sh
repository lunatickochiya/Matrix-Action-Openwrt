...
function patch_openwrt() {
        for i in $( ls mt798x-patch ); do
            echo Applying mt798x-patch $i
            patch -p1 < mt798x-patch/$i
        done
        }
...
# add luci
function add_mtk_mt798x_packages() {
echo "$(cat package-configs/mt798x-mtk-common.config)" >> .config
}
